...
std::basic_string<Char> join(const Sep& sep, InputIterator beg, InputIterator end) {
...
std::basic_string<Char> join(const Sep& sep, const std::vector<std::basic_string<Char>>& args) {
...
struct StringHelper<
    Char,
    String,
    typename std::enable_if<std::is_same<String, std::basic_string<Char>>::value>::type> {
    StringHelper(const std::basic_string<Char>& s) : s{s} {}
...
void replace(std::basic_string<Char>& s, const What& what, const By& by) {
    std::size_t pos = 0;
...
void prefix_with(std::basic_string<Char>& s, const What& what, const Char& by) {
    const StringHelper<Char, typename std::decay<What>::type> what_helper{what};
