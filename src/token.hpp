...
constexpr DWORD query() {
    return TOKEN_QUERY;
}

constexpr DWORD duplicate() {
...
Handle open_for_process(const Handle& process, DWORD permissions = default_permissions);

Handle open_for_current_process(DWORD permissions = default_permissions);
